...
export function AgentOrchestration({ killSwitch = 'inactive', onCreatedChangeRequest }: { killSwitch?: string; onCreatedChangeRequest?: () => void }) {
  ...
  return <section className="panel guard"><h2>Phase 3 Agent Orchestration</h2><p>Controlled agent requests create redacted artifacts only. Artifacts must become Phase 2 change requests before anything is applied.</p><form className="controlForm compact" onSubmit={submit}><label>Agent request title<input value={title} onChange={event => setTitle(event.target.value)} placeholder="Ask Igris to plan Phase 4" /></label><label>Target agent<select value={targetAgent} onChange={event => setTargetAgent(event.target.value)}><option>igris</option><option>atlas</option><option>sentinel</option><option>pulse</option><option>forge</option><option>clix</option><option>nexus</option><option>vector</option></select></label><label>Request kind<select value={kind} onChange={event => setKind(event.target.value)}><option value="mission.plan">mission.plan</option><option value="task.breakdown">task.breakdown</option><option value="code.review">code.review</option><option value="security.review">security.review</option><option value="architecture.proposal">architecture.proposal</option><option value="status.summary">status.summary</option></select></label><label>Prompt<textarea value={prompt} onChange={event => setPrompt(event.target.value)} placeholder="Bounded planning/review prompt" /></label><button type="submit">Create agent request</button></form>{message && <p className="statusLine">{message}</p>}<div className="cards"><article className="agent"><h3>Agent Request Queue</h3>{requests.length === 0 ? <p className="muted">No agent requests yet</p> : <div className="list">{requests.map(request => <div key={request.id} className="mini"><strong>{request.title}</strong><p className="muted">{request.targetAgent} · {request.status}</p><div className="actions">{request.status === 'pending_review' && <button onClick={() => void requestAction(request.id, 'approve')}>Approve</button>}{request.status === 'approved' && <button onClick={() => void requestAction(request.id, 'enqueue')}>Enqueue</button>}{request.status === 'queued' && <button onClick={() => void requestAction(request.id, 'dispatch-mock')}>Mock dispatch</button>}</div></div>)}</div>}</article><article className="agent"><h3>Agent Run Monitor</h3>{runs.length === 0 ? <p className="muted">No agent runs yet</p> : runs.slice(-5).reverse().map(run => <p key={run.id} className="muted">{run.targetAgent} · {run.status}</p>)}</article><article className="agent"><h3>Artifact Review</h3>{artifacts.length === 0 ? <p className="muted">No artifacts yet</p> : artifacts.slice(-5).reverse().map(artifact => <div key={artifact.id} className="mini"><p>{artifact.content.slice(0, 140)}</p><button onClick={() => void promote(artifact.id)}>Promote artifact</button></div>)}</article><article className="agent"><h3>Kill switch</h3><p>Status: {killSwitch}. Dispatch remains approval-gated.</p></article></div></section>;
}
